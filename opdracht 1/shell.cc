...
void new_file() {
  ...
  std::string path = "/Users/nathanhouwaart/CLionProjects/EOSopdracht1/1_os_level/" + filename + ".txt";
  char pathname[200];
  ...
  std::string text;
  std::cout << "text: ";
  std::getline(std::cin, text);               //user kan een string opgeven die in textfile geschreven wordt
  
  write(fd1, text.c_str(), text.size()); //write functie schrijft 'buffer' weg naar fd1. totaal schrijft hij het aantal bytes van buffer -1 weg.
